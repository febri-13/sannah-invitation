...
CREATE TABLE IF NOT EXISTS undangan.pengaturan (
...
CREATE INDEX IF NOT EXISTS idx_pengaturan_key ON undangan.pengaturan(key);

-- Row Level Security (RLS)
ALTER TABLE undangan.pengaturan ENABLE ROW LEVEL SECURITY;

-- Allow public read (anyone can read settings)
DROP POLICY IF EXISTS "Public read pengaturan" ON undangan.pengaturan;
CREATE POLICY "Public read pengaturan" ON undangan.pengaturan FOR SELECT USING (true);
...
INSERT INTO undangan.pengaturan (key, value, label, description) VALUES
(
